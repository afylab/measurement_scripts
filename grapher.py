...
class MainWindow(QtGui.QMainWindow):
    ...
    def denanify(self, x):
    	x = np.array(x)
    	xcomp = x[np.isfinite(x)]
        x[x == np.inf] = np.max(xcomp)
        x[x == -np.inf] = np.min(xcomp)
        x[np.isnan(x)] = 0.
        return x
    ...
    def update(self, cntx, signal):
        ...
        for i in range(d.shape[0]):
            self.dx[d[i, 0], d[i, 1]] = self.denanify(d[i, self.datacol1])
            self.dy[d[i, 0], d[i, 1]] = self.denanify(d[i, self.datacol2])
            self.volt[d[i, 0], d[i, 1]] = d[i, self.xcol]
            if (d[i, 0] == 0)and(d[i, 1] == 0):
                self.dx.fill(d[i, self.datacol1])
                self.dy.fill(d[i, self.datacol2])
    ...
    def update_params(self, cntx, signal):
        ...
        for p in params:
            ...
            if 'data1_col' in p:
                self.datacol1 = dict(params)['data1_col']
            if 'data2_col' in p:
                self.datacol2 = dict(params)['data2_col']
            if 'data1_label' in p:
                self.datalabel1 = dict(params)['data1_label']
            if 'data2_label' in p:
                self.datalabel2 = dict(params)['data2_label']
            if 'x_col' in p:
                self.xcol = dict(params)['x_col']
        if (hasattr(self, 'extent'))and(hasattr(self, 'pxsize')):
            print "creating graph"
            self.create_graph()
            self.cap1d.setLabels(title=self.datalabel1, left= self.datalabel1, bottom= "n_0 [V]")
            self.dis1d.setLabels(title=self.datalabel2, left= self.datalabel2, bottom= "n_0 [V]")
