...
RAMP_SPEED = 5.0  # volts per sec (not used?)
...
def create_file(dv, cfg, **kwargs): # try kwarging the vfixed
    ...
    dv.add_parameter('data1_col', 4)
    dv.add_parameter('data1_label', 'C/C_REF')
    dv.add_parameter('data2_col', 5)
    dv.add_parameter('data2_label', 'D/C_REF')
    dv.add_parameter('x_col',7)
...
def main():
    ...
    #mag = cxn.ami_430
    #mag.select_device()
    # tc = cxn.temperature_bf
    lck = lockin_select(cxn, str(cfg['lockin']))
    lck.select_device(0L)
    acbox = cxn.acbox
    ...
    for i in range(num_y):
        ...
        if np.any(mask == True):
            ...
            dc.set_voltage(dac_ch1,vec_x[start])
            dc.set_voltage(dac_ch2,vec_y[start])

            time.sleep(lockin_settings['tc']*50.0)
